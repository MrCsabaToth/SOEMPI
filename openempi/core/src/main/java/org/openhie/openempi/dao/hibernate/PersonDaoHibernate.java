...
public class PersonDaoHibernate extends UniversalDaoHibernate implements PersonDao
{
	...
	private String convertByteArrayToBlobString(byte[] blob) {
		...
		for(int i = 0; i < blob.length; i++) {
			Integer blobByte = Integer.valueOf(blob[i]);
			if (blobByte < 0)
				blobByte += 256;
			String blobByteString = Integer.toOctalString(blobByte);
			blobString.append("\\\\");	// Double escape is required, otherwise the string would be processed as UTF-8, and it won't be valid
			if (blobByteString.length() == 1)
				blobString.append("00");
			else if (blobByteString.length() == 2)
				blobString.append("0");
			blobString.append(blobByteString);
		}
		blobString.append("'");
