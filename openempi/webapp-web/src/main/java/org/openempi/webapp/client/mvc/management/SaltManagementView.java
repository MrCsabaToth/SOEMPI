...
public class SaltManagementView extends View
{
	...
	public SaltManagementView(Controller controller) {
		super(controller);
	}

	@Override
	protected void handleEvent(AppEvent event) {
		if (event.getType() == AppEvents.SaltManagementView) {
			initUI();
		} else if (event.getType() == AppEvents.SaltManagementAddRenderData) {
			container.layout();
			status.hide();
			addButton.unmask();
		}
	...
	private void initUI() {
		...
		addButton = new Button("Generate Salts", new SelectionListener<ButtonEvent> () {
			@Override
			public void componentSelected(ButtonEvent ce) {
				controller.handleEvent(new AppEvent(AppEvents.SaltManagementAddInitiate));
				status.show();
				addButton.mask();
			}
		});
